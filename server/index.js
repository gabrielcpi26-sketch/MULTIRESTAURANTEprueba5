// server/index.js — carga robusta de .env para Stripe
...
// Cargar .env desde /server/.env o desde la carpeta raíz
let envPath = path.resolve(__dirname, ".env");
let result = dotenv.config({ path: envPath, debug: true });

if (!result.parsed || Object.keys(result.parsed).length === 0) {
  envPath = path.resolve(__dirname, "../.env");
  result = dotenv.config({ path: envPath, debug: true });
}

console.log("[.env] usando archivo:", envPath);
console.log("[.env] ¿Stripe key presente?:", !!(process.env.STRIPE_SECRET_KEY));
// server/index.js
